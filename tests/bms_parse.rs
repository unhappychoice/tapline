...
fn tapline_bin() -> &'static str {
    env!("CARGO_BIN_EXE_tapline")
}
...
fn sample_charts_have_expected_shape() {
    ...
    for (rel, title, lanes, level, difficulty) in expected {
        ...
        assert!(
            text.contains(&format!("#PLAYLEVEL {}", level)),
            "{}: level mismatch",
            rel
        );
        assert!(
            text.contains(&format!("#DIFFICULTY {}", difficulty)),
            "{}: difficulty mismatch",
            rel
        );
        let highest_channel = match lanes {
            ...
            5 => "15",
            7 => "19",
            _ => panic!("unexpected lanes"),
        };
        assert!(
            text.contains(highest_channel),
            "{}: expected a channel {} line",
            rel,
            highest_channel
        );
    }
