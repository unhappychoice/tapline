...
impl SampleBank {
    pub fn new(wav_paths: &HashMap<u32, PathBuf>) -> Self {
        let (stream, handle, enabled) = silence_stderr(|| {
    ...
    pub fn play(&self, id: u32) {
        ...
        let Some(s) = self.samples.get(&id) else { return; };
...
fn decode(path: &PathBuf) -> anyhow::Result<DecodedSample> {
    ...
    Ok(DecodedSample { channels, sample_rate, data })
}

#[cfg(unix)]
fn silence_stderr<F, R>(f: F) -> R where F: FnOnce() -> R {
    use std::os::fd::AsRawFd;
    let saved = unsafe { libc::dup(2) };
    let devnull = std::fs::OpenOptions::new().write(true).open("/dev/null").ok();
    if let Some(ref d) = devnull {
        if saved >= 0 { unsafe { libc::dup2(d.as_raw_fd(), 2); } }
    }
    let out = f();
    if saved >= 0 {
...
#[cfg(not(unix))]
fn silence_stderr<F, R>(f: F) -> R where F: FnOnce() -> R { f() }
