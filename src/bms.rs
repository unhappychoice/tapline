...
pub fn read_meta(path: &Path) -> Result<ChartMeta> {
    ...
    Ok(ChartMeta {
        ...
        title: pass.title, artist: pass.artist, bpm: pass.bpm,
        playlevel: pass.playlevel, difficulty: pass.difficulty,
...
fn build_chart(pass: HeaderPass, raw: Vec<(u32, String, String)>, dir: PathBuf, lead_in_ms: f64) -> Result<Chart> {
    ...
    for (measure, ch, data) in &raw {
        ...
        materialize_channel(*measure, ch, data, measure_ms, lead_in_ms, &mut lanes, &mut notes, &mut bgm);
    }
    let lane_count = lanes.count();
...
fn materialize_channel(measure: u32, ch: &str, data: &str, measure_ms: f64, lead_in_ms: f64,
    lanes: &mut LaneSet, notes: &mut Vec<Note>, bgm: &mut Vec<BgmEvent>) {
    ...
    for (i, slot) in slots.iter().enumerate() {
        ...
        if let Some(lane) = channel_to_lane(ch) {
            ...
            bgm.push(BgmEvent { time_ms: t, keysound: *slot });
        }
    }
}
...
impl Default for HeaderPass {
    fn default() -> Self {
        Self {
            ...
            bpm: 130.0,
            playlevel: None, difficulty: None,
...
impl HeaderPass {
    fn absorb(&mut self, line: &str) {
        ...
        match up.as_str() {
            ...
            "BPM"        => { if let Ok(v) = val.trim().parse() { self.bpm = v; } }
            "PLAYLEVEL"  => { if let Ok(v) = val.trim().parse() { self.playlevel = Some(v); } }
            "DIFFICULTY" => { if let Ok(v) = val.trim().parse() { self.difficulty = Some(v); } }
            _ => {
...
impl LaneSet {
    fn observe(&mut self, ch: &str) {
        if let Some(lane) = channel_to_lane(ch) { self.0.insert(lane); }
    }
    fn count(&self) -> usize {
...
fn resolve_wavs(dir: &Path, defs: &HashMap<u32, String>) -> HashMap<u32, PathBuf> {
    defs.iter().filter_map(|(id, name)| {
        let direct = dir.join(name);
        if direct.exists() { return Some((*id, direct)); }
        let stem = Path::new(name).file_stem().and_then(|s| s.to_str()).unwrap_or(name);
        ["wav", "ogg", "mp3", "flac"].iter()
