...
pub fn draw(out: &mut Stdout, game: &Game, now_ms: f64) -> anyhow::Result<()> {
    ...
    if bottom <= top + 4 { return Ok(()); }
    let lane_height = bottom - top;

    queue!(out, terminal::BeginSynchronizedUpdate)?;

    let title = if game.chart.title.is_empty() { "TAPLINE".to_string() } else { game.chart.title.clone() };
    queue!(out, cursor::MoveTo(cols.saturating_sub(title.chars().count() as u16) / 2, 1),
        SetForegroundColor(Color::Magenta), Print(&title), ResetColor)?;

    let hud = format!("score {:>6}    combo {:>3}    acc {:>5.1}%",
    ...
    for lane in 0..lanes {
        ...
        for r in top..bottom {
            queue!(out, cursor::MoveTo(lx, r), SetForegroundColor(Color::DarkGrey),
                Print("│      "))?;
        }
    ...
    for x in x0..(x0 + field_width) {
        queue!(out, cursor::MoveTo(x, judgment_row),
            SetForegroundColor(Color::White), Print("═"))?;
    }
    queue!(out, ResetColor)?;
...
pub fn draw_intro(out: &mut Stdout, game: &Game, countdown_ms: f64, audio_on: bool) -> anyhow::Result<()> {
    let (cols, rows) = terminal::size()?;
    queue!(out, terminal::BeginSynchronizedUpdate)?;
    let title = if game.chart.title.is_empty() { "T A P L I N E".to_string() } else { game.chart.title.clone() };
    queue!(out, cursor::MoveTo(cols.saturating_sub(title.chars().count() as u16) / 2, rows / 2 - 3),
    ...
    queue!(out, cursor::MoveTo(cols.saturating_sub(count.len() as u16) / 2, rows / 2 + 2),
        SetForegroundColor(Color::Yellow),
        style::SetAttribute(style::Attribute::Bold),
        Print(count),
        style::SetAttribute(style::Attribute::Reset), ResetColor)?;
    let badge = if audio_on { "audio on" } else { "silent" };
    queue!(out, cursor::MoveTo(cols.saturating_sub(badge.len() as u16) / 2, rows - 2),
        SetForegroundColor(Color::DarkGrey), Print(badge), ResetColor)?;
    queue!(out, terminal::EndSynchronizedUpdate)?;
    out.flush()?;
    Ok(())
}
...
pub fn draw_result(out: &mut Stdout, game: &Game) -> anyhow::Result<()> {
    ...
    let line = |o: &mut Stdout, text: String, color: Color, yy: u16| -> anyhow::Result<()> {
        queue!(o, cursor::MoveTo(cols.saturating_sub(text.chars().count() as u16) / 2, yy),
            SetForegroundColor(color), Print(text), ResetColor)?;
        Ok(())
    };
    line(out, "R E S U L T".to_string(), Color::Magenta, y)?; y += 2;
    line(out, format!("score       {:>6}", game.score),     Color::White,  y)?; y += 1;
    line(out, format!("max combo   {:>6}", game.max_combo), Color::Cyan,    y)?; y += 1;
    line(out, format!("accuracy    {:>5.1}%", game.accuracy()), Color::Green, y)?; y += 2;
    line(out, format!("perfect {}   great {}   good {}   miss {}",
            game.perfect, game.great, game.good, game.miss),
        Color::DarkGrey, y)?; y += 2;
