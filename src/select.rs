...
pub fn scan(root: &Path) -> Vec<ChartMeta> {
    ...
    walk(root, 0, &mut out);
    out.sort_by(|a, b| {
        a.title.to_ascii_lowercase().cmp(&b.title.to_ascii_lowercase())
...
fn walk(dir: &Path, depth: usize, out: &mut Vec<ChartMeta>) {
    ...
    for entry in entries.flatten() {
        ...
        if ty.map(|t| t.is_dir()).unwrap_or(false) {
            ...
            if let Ok(meta) = bms::read_meta(&path) { out.push(meta); }
        }
    }
}
...
pub fn run(out: &mut Stdout, charts: &[ChartMeta]) -> Result<Option<PathBuf>> {
    if charts.is_empty() { return Ok(None); }
            ...
            if let Event::Key(k) = event::read()? {
                ...
                match k.code {
                    ...
                    KeyCode::Down | KeyCode::Char('j') => {
                        if cursor_i + 1 < charts.len() { cursor_i += 1; }
                    }
...
fn draw(out: &mut Stdout, charts: &[ChartMeta], cursor_i: usize, scroll: &mut usize) -> Result<()> {
    ...
    queue!(out, terminal::BeginSynchronizedUpdate, terminal::Clear(terminal::ClearType::All))?;
    ...
    queue!(out, cursor::MoveTo(cols.saturating_sub(subtitle.chars().count() as u16) / 2, 2),
        SetForegroundColor(Color::DarkGrey), Print(&subtitle), ResetColor)?;

    let list_top = 4u16;
    let list_bottom = rows.saturating_sub(2);
    let view_rows = list_bottom.saturating_sub(list_top) as usize;
    if view_rows == 0 { queue!(out, terminal::EndSynchronizedUpdate)?; out.flush()?; return Ok(()); }

    if cursor_i < *scroll { *scroll = cursor_i; }
    if cursor_i >= *scroll + view_rows { *scroll = cursor_i + 1 - view_rows; }

    for row in 0..view_rows {
        let i = *scroll + row;
        if i >= charts.len() { break; }
        let m = &charts[i];
        ...
        let title = if m.title.is_empty() {
            m.path.file_name().and_then(|s| s.to_str()).unwrap_or("(untitled)").to_string()
        } else {
            m.title.clone()
        };
        let artist = if m.artist.is_empty() { "".to_string() } else { format!("  — {}", m.artist) };
        ...
        if selected { queue!(out, style::SetAttribute(style::Attribute::Bold))?; }
...
fn format_badge(m: &ChartMeta) -> String {
    ...
    let dif = match difficulty_label(m.difficulty) { "" => "".to_string(), s => format!(" {}", s) };
